# 脚本复用目录（Reuse Catalog）— boss-zhipin-copilot

> **本文件是「任务 → 该复用的内置脚本」唯一权威映射。** 任何 Agent 在动手写新脚本前，**必须先查本表**。
> 配合 SKILL.md 的「🛑 脚本复用铁律」。若表中有对应内置脚本，**必须复用，禁止重写等价脚本**。
> 若表中标注「缺口」且该能力确无内置脚本，按铁律③首次遇到时**新建并收编进 `scripts/`**，后续一律复用。
